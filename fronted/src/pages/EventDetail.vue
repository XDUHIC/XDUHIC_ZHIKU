    ...
    <div class="event-detail-page">
      <div class="page-header">
        ...
        <div class="header-content">
          <h1 class="page-title">{{ detail.title || '活动详情' }}</h1>
          <div class="page-meta">
            <span v-if="detail.startTime || detail.start_time" class="meta-item">
              <i class="fas fa-calendar-alt"></i>
              {{ formatDateTime(detail.startTime || detail.start_time) }}
            </span>
...
const fetchDetail = async () => {
  ...
  try {
    ...
    // 添加时间戳参数确保获取最新版本
    const { data } = await axios.get(`/api/portal/events/${id}?_t=${Date.now()}`)

    const e = data?.data || data || {}
    detail.value = e

    // 处理Markdown内容
    const mdUrl = e.textUrl || e.text_url || e.markdownUrl || e.mdUrl || e.contentUrl || e.content_path || e.contentPath || e.md_path || e.file_url || e.documentUrl

    if (mdUrl) {
      await loadMarkdownAndRender(mdUrl)
    } else if (e.content) {
      // 如果content是URL路径，则加载文件；否则直接渲染
      if (typeof e.content === 'string' && (e.content.endsWith('.md') || e.content.includes('/uploads/') || e.content.startsWith('uploads/'))) {
        await loadMarkdownAndRender(e.content)
      } else {
        await loadMarkdownAndRender(e.content)
      }
    } else {
      htmlContent.value = '<p style="color: orange;">暂未找到文档内容</p>'
    }
  } catch (err) {
    console.error('获取活动详情失败:', err)
    detail.value = {}
    htmlContent.value = '<p style="color: red;">加载活动详情失败</p>'
  } finally {
    loading.value = false
  }
}
